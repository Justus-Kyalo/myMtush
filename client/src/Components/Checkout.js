  ...
  render() {
    ...
    return(
      <div className="cart-totalinformation-container">
        <span className="cart-totalprice"> Total Price: Ksh{total.toFixed(2)} </span>
        <p className="cart-totalprice"> Total Quantity: {totalQuantity}</p>
        {total.toFixed(2) > 1000 ? <div className="free-shipping">Your Shipping is Free! </div> : <div className="free-shipping">Plus Shipping: Ksh 200 <p className="from-shipping">Only Ksh {(1000 - total.toFixed(2)).toFixed(2)} away from Free Shipping</p></div>}
      </div>
    )
  }
}
